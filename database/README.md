...
ETL d'ingestion des CSV. Projet uv **indépendant** (package `obrail_database`), consommé par l'API
(qui n'en importe que les modèles) et déployable seul via son image Docker.
...
| ETL | `src/obrail_database/etl/` | ingestion CSV (`loaders`), résolution des jointures (`resolve`), vues (`views`), CLI (`run`) |
...
# Ingestion des CSV de data/ (idempotente ; --force pour réécraser).
...
L'image `obrail-database` est **auto-portante** : elle provisionne le schéma et embarque les CSV.
Construite depuis la **racine du dépôt** (elle copie `data/`) :

```bash
docker build -t obrail-database -f database/Dockerfile .   # depuis la racine
